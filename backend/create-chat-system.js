...
async function createChatTables() {
  try {
    ...
    console.log('🔨 Creando tablas para el sistema de chat interno...\n');
    ...
    const [projects] = await connection.execute(
      'SELECT id, name FROM projects WHERE id NOT IN (SELECT project_id FROM chat_rooms WHERE type = "project")'
    );
    ...
    if (generalRoom.length === 0) {
      const [roomResult] = await connection.execute(
        'INSERT INTO chat_rooms (project_id, name, type) VALUES (0, "Chat General - Admins", "general")'
      );
